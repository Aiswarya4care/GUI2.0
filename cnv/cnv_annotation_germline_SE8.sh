...
{{bedtools}} intersect -a ${line}/${line}.bam_CNVs -b {{GUIpath}}/bed_files/cnv_bed_files/cnv_capturing_bedfiles/indiegene_whole-gene.bed -loj | sort -V | awk -F"\t" '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$9}' | awk -vOFS="\t" '$1=$1; BEGIN { str="Chromosome Start End Predicted_copy_number Type_of_alteration Gene"; split(str,arr," "); for(i in arr) printf("%s\t", arr[i]);print}' | awk '$6 != "."' > ./${line}"_Indiegene_cnv_output.txt"
