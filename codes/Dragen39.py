...
### running the bs command based on sample extension
    ...
    if ("-SE8-" in ':'.join(samples))*("-CE-" in ':'.join(samples)) ==1:
        ...
        bscmd="bs launch application -n \"DRAGEN Enrichment\" --app-version 3.9.5 -o project-id:" + pid + " -o app-session-name:"+ appsess +"  -l "+ appsess +" -o vc-type:1 -o ht-ref:hg19-altaware-cnv-anchor.v8 -o fixed-bed:custom -o target_bed_id:" + str(bed_id) + "-o input_list.sample-id:$bsids -o picard_checkbox:1 -o af-filtering:1  -o vc-af-call-threshold:5 -o vc-af-filter-threshold:10 -o sv_checkbox:1 -o sq-filtering:1 -o tmb:1 -o vc-hotspot:26309592242 -o baseline-noise-bed:26693875133 -o vcf-site-filter:1 -o cnv_checkbox:1 -o cnv_ref:1 -o cnv_segmentation_mode:cbs -o cnv-filter-qual:50.0 -o cnv-baseline-id:" + str(cnv_baseline_id) + "-o cnv_gcbias_checkbox:1 -o hla:1 -o commandline-disclaimer:true -o arbitrary:\"--read-trimmers:adapter --trim-adapter-read1\" -o additional-file:25600057590 -o automation-sex:unkown"
